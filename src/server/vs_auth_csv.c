			...
			while (fgets(line, LINE_LEN - 2, file) != NULL ) {
				...
				if (raw == 0) {
					if (strncmp(&line[0], "username", 8) != 0)
						break;
					if (strncmp(&line[9], "password", 8) != 0)
						break;
					if (strncmp(&line[18], "UID", 3) != 0)
						break;
					if (strncmp(&line[22], "real name", 9) != 0)
						break;
				} else {
					...
					next_comma = col;
					new_user->username = strndup(&line[prev_comma],
							next_comma - prev_comma);
					prev_comma = next_comma + 1;
					...
					next_comma = col;
					new_user->password = strndup(&line[prev_comma],
							next_comma - prev_comma);
					prev_comma = next_comma + 1;
					...
					next_comma = col;
					new_user->realname = strndup(&line[prev_comma],
							next_comma - prev_comma);
